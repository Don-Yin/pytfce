"""eTFCE-accelerated pTFCE: union-find cluster retrieval replaces repeated CCL.

Implements the cluster retrieval algorithm from:
    Chen, X., Weeda, W., Nichols, T.E., & Goeman, J.J. (2026).
    eTFCE: Exact Threshold-Free Cluster Enhancement via Fast Cluster Retrieval.
    arXiv:2603.03004.

Instead of running connected-component labeling (CCL) at each of Nh thresholds
(O(Nh x N)), we build a union-find forest incrementally as the threshold
descends, snapshotting cluster sizes at each h_grid level. This replaces
Nh separate CCL passes with a single incremental construction.
"""
...
def ptfce_etfce(
    ...
    """Run pTFCE using eTFCE-style incremental union-find cluster retrieval.

    Builds the cluster hierarchy incrementally as the threshold descends,
    snapshotting cluster structure at each pTFCE threshold level. This
    eliminates the need for separate CCL at each threshold.
    """
    ...
    if verbose and progress_queue is None:
        print(f"  [eTFCE-pTFCE] Rd={Rd:.1f}, V={V}, Nh={Nh}, "
    ...
    sx, sy, sz = Z.shape
    strides = np.array([sy * sz, sz, 1], dtype=np.intp)
    ...
    for j in range(Nh - 1, -1, -1):
        ...
        if progress_queue and j % 10 == 0:
            progress_queue.put({"name": "eTFCE-pTFCE",
                                "current": Nh - j, "total": Nh,
                                "phase": "thresholds"})
    ...
    if progress_queue:
        progress_queue.put({"name": "eTFCE-pTFCE", "current": Nh,
                            "total": Nh, "phase": "done"})
    ...
    return {
        ...
        "diagnostics": {
            ...
            "n_effective_thresholds": n_ccl_equiv,
        },
    }
